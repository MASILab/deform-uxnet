...
train_images = img_list[:341]
train_labels = label_list[:341]

valid_images = img_list[341:]
valid_labels = label_list[341:]
...
# train_images = sorted(glob.glob(os.path.join(img_path, "*.nii.gz")))[:10]
# train_labels = sorted(glob.glob(os.path.join(label_path, "*.nii.gz")))[:10]

# valid_images = sorted(glob.glob(os.path.join(img_path, "*.nii.gz")))[80:]
# valid_labels = sorted(glob.glob(os.path.join(label_path, "*.nii.gz")))[80:]
...
train_files, val_files = data_dicts[:len(train_images)], data_dicts[len(train_images):]
...
# pretrained_path = os.path.join('/nfs/masi/leeh43/monai_seg/FLARE_repcovxnet_MIDL_lr_0.0001_AdamW_all_bz_1_sample_2_21x21x21_conv_matrix/best_metric_model.pth')
...
root_dir = os.path.join('/nfs/masi/leeh43/monai_seg/FLARE_deformuxnet_MIDL_lr_0.0001_AdamW')
